<%@ attribute name="result" required="true" rtexprvalue="true" type="fr.openwide.alfresco.query.web.form.result.FormQueryResult" %>
...
<c:if test="${not empty result}">
	<table class="table table-striped table-bordered table-hover formquery-result">
		<thead>
			<c:forEach var="column" items="${result.columns}" varStatus="columnStatus">
				<th>
					<spring:message message="${column.label}"/>
					<c:if test="${not empty column.comparator}">
						<c:choose>
