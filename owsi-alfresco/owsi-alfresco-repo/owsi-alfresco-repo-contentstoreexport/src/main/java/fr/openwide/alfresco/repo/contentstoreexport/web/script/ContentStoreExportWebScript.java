...
@GenerateWebScript(
		...
		swaggerParameters={
			...
			@SwaggerParameter(name="writeToDisk", description="Emplacement où écrire sur disque le Zip (défaut renvoie juste le Zip)"),
			@SwaggerParameter(name="writeToAlfresco", description="Emplacement où écrire sur disque le Zip (défaut renvoie juste le Zip)"),
...
public class ContentStoreExportWebScript extends AbstractWebScript implements ApplicationContextAware {
	...
	public void execute(WebScriptRequest req, WebScriptResponse resp) throws IOException {
		...
		String fileName = "contentstoreexport-" 
		...
		try {
			contentStoreExportService.export(outputStream, fileName, params);
			outputStream.flush();
		} finally {
			outputStream.close();
		}
